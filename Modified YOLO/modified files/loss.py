...
class KeypointLoss(nn.Module):
    ...
    def forward(
        ...
        total_kpts = torch.sum(kpt_mask != 0 ) + torch.sum(kpt_mask == 0)
        visible_kpts = torch.sum(kpt_mask != 0) + 1e-9
        kpt_loss_factor = total_kpts / visible_kpts

        #OKS
        e = d_sq / ((2 * self.sigmas).pow(2) * (area + 1e-9) * 2)
        oks_loss = 1 - torch.exp(-e)

        combined_loss = ((0.9 * d_abs) + (0.1 * oks_loss)) * kpt_mask
        return kpt_loss_factor * combined_loss.mean()
...
class v8PoseLoss(v8DetectionLoss):
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int = 10):  # model must be de-paralleled
        ...
        sigmas = torch.from_numpy(OKS_SIGMA).to(self.device) if is_pose else (torch.ones(nkpt, device=self.device) / nkpt) / 10
        print(f"Initializing Custom Keypoint Loss with sigmas: {sigmas}")
